...
namespace GraphFunc
{
    class Edge
    {
        ...
        {
            var (crossType, t) = e.Intersect(this);
        ...
        {
            float v1_x = _dest.X - _source.X;
            float v1_y = _dest.Y - _source.Y;
            float v2_x = p.X - _source.X;
            float v2_y = p.Y - _source.Y;
            if ((v2_x * v1_y - v2_y * v1_x) == 0)
                if ((_source.X < p.X && p.X < _dest.X) || (_dest.X < p.X && p.X < _source.X))
                    return true;
            return false;
        }
    }
